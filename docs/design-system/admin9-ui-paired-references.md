...
| Main navigation and list | Home/Account destinations, guest and signed-in account rows, legal/support rows | Same page bar, navigation language, row/section rhythm, selected state and danger boundary | Safe area, predictive/edge back, route transition, accessibility focus |
...
| Dialog and feedback | Confirmation, destructive action, disabled command, loading, empty and recoverable error | Same modal hierarchy, action order, tones, loading/empty/error composition | Modal focus, dismissal gesture, live-region announcement, safe area |
...
| Long Chinese/reflow | Error and notice | Identity/list values | Effective-state explanation | Error/recovery message |
| Focus/pressed | Field and main action annotations | Row/nav annotations | Row/switch annotations | Modal actions/close annotations |
| Disabled | Submit/field evaluation requirement | Row evaluation requirement | Switch evaluation requirement | Disabled command shown |
| Loading | Submission contract | Page state evaluation requirement | Persistence contract | Determinate/indeterminate shown |
| Empty | Not applicable to form data | List empty state | Not applicable | Empty state shown |
| Error | Field and service boundary | Recoverable list requirement | Persistence error requirement | Persistent recoverable error shown |
