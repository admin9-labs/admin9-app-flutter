...
Core owns every visible interactive platform choice. Feature code MUST NOT call `Platform.isIOS`, select Material/Cupertino controls, change route builders, or import platform implementation files. Business pages MAY use non-interactive Flutter layout primitives: `Widget`, `Text`, `Row`, `Column`, `Stack`, `Wrap`, `Flex`, `Expanded`, `Flexible`, `Padding`, `Align`, `Center`, `SizedBox`, `ConstrainedBox`, `LayoutBuilder`, `MediaQuery`, `SafeArea`, `SingleChildScrollView`, `ListView`, `CustomScrollView`, `Sliver*`, `Form`, `FocusTraversalGroup`, and `Semantics`. Interactive commands, fields, selections, feedback, dialogs, sheets, lists, page bars, and navigation go through `App*` APIs.
