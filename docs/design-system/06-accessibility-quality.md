...
The App host has four exact Core-internal seams: `admin9_app.dart` and `app_routes.dart` may import the internal theme resolver; `app_routes.dart` may import the Gallery registry and Gallery page. These paths are explicit AST allowlist pairs, not a directory wildcard. The theme resolver and Gallery remain absent from `lib/admin9_ui.dart`, so Business cannot bypass the Brand entry or register debug tooling.
